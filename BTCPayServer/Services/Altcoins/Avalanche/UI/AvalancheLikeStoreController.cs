...
using BTCPayServer.Services.Altcoins.Avalanche.Filters;
...
namespace BTCPayServer.Services.Altcoins.Avalanche.UI
{
    
    [Route("stores/{storeId}/avalanchelike")]
    [OnlyIfSupportAvalanche]
    [Authorize(AuthenticationSchemes = AuthenticationSchemes.Cookie)]
    [Authorize(Policy = Policies.CanModifyStoreSettings, AuthenticationSchemes = AuthenticationSchemes.Cookie)]
    public class AvalancheLikeStoreController : Controller
    {
        private readonly StoreRepository _storeRepository;
        private readonly BTCPayNetworkProvider _btcPayNetworkProvider;

        public AvalancheLikeStoreController(StoreRepository storeRepository,
        ...
        [HttpGet()]
        public IActionResult GetStoreAvalancheLikePaymentMethods()
        {
            var eth = StoreData.GetSupportedPaymentMethods(_btcPayNetworkProvider)
                .OfType<AvalancheSupportedPaymentMethod>();

            var excludeFilters = StoreData.GetStoreBlob().GetExcludedPaymentMethods();
            var ethNetworks = _btcPayNetworkProvider.GetAll().OfType<AvalancheBTCPayNetwork>();

            var vm = new ViewAvalancheStoreOptionsViewModel() { };
            ...
            foreach (var network in ethNetworks)
            {
                ...
                vm.Items.Add(new ViewAvalancheStoreOptionItemViewModel()
                {
                    CryptoCode = network.CryptoCode,
                    Enabled = matchedPaymentMethod != null && !excludeFilters.Match(paymentMethodId),
                    IsToken = network is ERC20AvalancheBTCPayNetwork,
        ...
        [HttpGet("{cryptoCode}")]
        public IActionResult GetStoreAvalancheLikePaymentMethod(string cryptoCode)
        {
            var eth = StoreData.GetSupportedPaymentMethods(_btcPayNetworkProvider)
                .OfType<AvalancheSupportedPaymentMethod>();
            
            var network = _btcPayNetworkProvider.GetNetwork<AvalancheBTCPayNetwork>(cryptoCode);
            if (network is null)
            {
                return NotFound();
            }
            ...
            return View(new EditAvalanchePaymentMethodViewModel()
            {
        ...
        [HttpPost("{cryptoCode}")]
        public async Task<IActionResult> GetStoreAvalancheLikePaymentMethod(string cryptoCode,
            EditAvalanchePaymentMethodViewModel viewModel)
        {
            var network = _btcPayNetworkProvider.GetNetwork<AvalancheBTCPayNetwork>(cryptoCode);
            if (network is null)
            {
            ...
            var currentPaymentMethod = StoreData.GetSupportedPaymentMethods(_btcPayNetworkProvider)
                .OfType<AvalancheSupportedPaymentMethod>().SingleOrDefault(method =>
                    method.PaymentId == paymentMethodId);
            ...
            currentPaymentMethod ??= new AvalancheSupportedPaymentMethod();
            ...
            return RedirectToAction("GetStoreAvalancheLikePaymentMethods", new {storeId = store.Id});
        }
    }

    public class EditAvalanchePaymentMethodViewModel
    {
    ...
    public class ViewAvalancheStoreOptionsViewModel
    {
        public List<ViewAvalancheStoreOptionItemViewModel> Items { get; set; } =
            new List<ViewAvalancheStoreOptionItemViewModel>();
    }

    public class ViewAvalancheStoreOptionItemViewModel
    {
